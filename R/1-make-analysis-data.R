...
p1s1[, setdiff(names(p1s1), names(p1s2))] <- NULL
p1s1[, setdiff(names(p1s1), names(p2))]   <- NULL

p1s2[, setdiff(names(p1s2), names(p1s1))] <- NULL
p1s2[, setdiff(names(p1s2), names(p2))]   <- NULL

p2[, setdiff(names(p2), names(p1s1))]   <- NULL
p2[, setdiff(names(p2), names(p1s2))]   <- NULL

# checking. these should all equal TRUE
# all(sort(names(p1s1)) == sort(names(p1s2)))
# all(sort(names(p1s1)) == sort(names(p2)))
# all(sort(names(p1s2)) == sort(names(p2)))

analysis <- rbind(p1s1, p1s2, p2)
...
analysis$edu <- factor(analysis$edu, 
  labels = c("HSorLess", "HSgrad", "CollegeOrMore"))
