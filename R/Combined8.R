...
Eric <- subset(eric, select = -c(
        ...
        total_cpd_week6, study_cpd_week6, non_study_cpd_week6, tne_nmolperml_week6, tot_nnal_pmolperml_week6, FTND_score_week6))

Immed <- subset(immed, select = -c(
        WISDM_affective, WISDM_affiliative, WISDM_auto, WISDM_cog, WISDM_cue, WISDM_craving, WISDM_lossofcontrol, WISDM_social, WISDM_taste, WISDM_tolerance, WISDM_weight, 
        CES_satisfaction, CES_reward, CES_aversion, CES_enjoy, CES_cravingreduction,
...
Doro <- subset(doro, select = -c(
        WISDM_affective, WISDM_affiliative, WISDM_auto, WISDM_cog, WISDM_cue, WISDM_craving, WISDM_lossofcontrol, WISDM_social, WISDM_taste, WISDM_tolerance, WISDM_weight, 
        CES_satisfaction, CES_reward, CES_aversion, CES_enjoy, CES_cravingreduction, 
        nnal_visit8,tne_visit8, totalcpd_visit8, studycpd_visit8, nscpd_visit8, ftndwcpd_visit8, SMAST_total_visit92, DAST_total_visit92, nnal_visit20,tne_visit20, totalcpd_visit20, studycpd_visit20, nscpd_visit20, ftndwcpd_visit20 ))

#Eric is 661 by 26 ???????????? 20
#Immed is 642 by 34
#Doro is 429 by 34 (compliant from immed)
...
Etab <- make_tree(cbind(Z_EE, Eric), "Z_EE", nn) %>% as.data.frame()
...
legend("topleft", col = c("black", "red"), legend = c("Dorothy's data", "Eric's data"), pch = c(16, 16))
#putting everything in the same order
eric <- eric[,order(names(eric))]
...
###########PLOT 5##########
tree <- rtree(Z_DD, Doro)
Dsmk2 <- subset(Doro, select=c(FTND, age))
tree2 <- rtree(Z_DD, Dsmk2)
dataT <- Doro
dataT$Z <- Z_DD
